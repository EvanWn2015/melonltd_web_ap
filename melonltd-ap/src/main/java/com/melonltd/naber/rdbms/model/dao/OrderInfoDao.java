...
public interface OrderInfoDao extends JpaRepository<OrderInfo, String> {
	...
	@Query("SELECT o FROM OrderInfo o WHERE (o.fetchDate BETWEEN ?2 AND ?3) AND o.enable='Y' AND o.status!='FAIL' AND o.accountUUID IN(SELECT a.accountUUID FROM AccountInfo a WHERE a.phone LIKE %?1)")
	public List<OrderInfo> findByPhoneAndBetweenDate (String phone, String startDate, String endDate );
